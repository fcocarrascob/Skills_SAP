"""Backend de Combinaciones de Carga — SAP2000 (COM directo)."""

from sap_connection import SapConnection


class CombosBackend:
    """CRUD de combinaciones de carga (RespCombo) en SAP2000."""
    ...
    def get_load_cases(self) -> list:
        """Retorna lista de nombres de Load Cases."""
    ...
    def get_combinations(self) -> list:
        """
        Retorna lista de dicts con cada combinación.
        Estructura: [{'name': str, 'type': int, 'items': {'CASE': factor}}, ...]
        """
        SapModel = self.sap_model
        combos = []
        try:
    ...
    def _clear_combo_items(self, name: str):
        """Elimina todos los load cases de una combinación existente (para reasignación idempotente)."""
        SapModel = self.sap_model
        try:
            ret_list = SapModel.RespCombo.GetCaseList(name)
            if ret_list[-1] == 0 and ret_list[0] > 0:
    ...
    def delete_combination(self, name: str) -> bool:
        """Elimina una combinación por nombre. Retorna True si tuvo éxito."""
        SapModel = self.sap_model
        try:
    ...
    def push_combinations(self, combos_data: list) -> int:
        """
        Envía combinaciones a SAP2000.
        combos_data: [{'name': str, 'type': int, 'items': {'CASE': factor}}, ...]
