...
## Goal
Create the MCP server entry point and COM bridge that connects to a local SAP2000 instance, with `connect_sap2000`, `disconnect_sap2000`, and `get_model_info` tools.

## Prerequisites
Make sure that the user is currently on the `feat/sap2000-copilot-skill` branch before beginning implementation.
If not, move them to the correct branch. If the branch does not exist, create it from main.

### Step-by-Step Instructions

#### Step 1.1: Create project structure and dependencies
- [ ] Create the folder `mcp_server/` in the workspace root.
- [ ] Copy and paste code below into `mcp_server/requirements.txt`:
...
- [ ] Copy and paste code below into `mcp_server/__init__.py`:
...
#### Step 1.2: Create the SAP2000 COM Bridge
- [ ] Copy and paste code below into `mcp_server/sap_bridge.py`:
...
#### Step 1.3: Create the MCP Server entry point
- [ ] Copy and paste code below into `mcp_server/server.py`:
...
mcp = FastMCP(
    "sap2000",
    description="Bridge to SAP2000 structural analysis software via local COM.",
)
...
- [ ] Folder `mcp_server/` exists with `__init__.py`, `requirements.txt`, `sap_bridge.py`, `server.py`
- [ ] `pip install -r mcp_server/requirements.txt` completes without errors
- [ ] Running `python mcp_server/server.py` starts without import errors (press Ctrl+C to stop)
